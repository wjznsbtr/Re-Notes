...
1. 打开日志记录，找到最后一个异常发生的地址，然后重启程序，观察异常发生地址变化，看到最后一次异常发生的地址后，给主程序区段依次设置内存访问断点，直到成功断下为止。
2. 这是第一个方法的简化版，给 KiUserExceptionDispatcher 这个异常分发的api设置条件记录断点，堆栈中esp+14位置对应的地址就是异常发生地址。第一遍用条件断点输出日志文本 Exception address = {[esp + 14]} ；第二遍把断点条件改成最后一个异常发生的地址，断下来设内存断点。这样就不用人肉眼盯着了，缓解视疲劳。
...
即某些壳在处理OEP代码的时候,会把**OEP**处固定的代码**NOP**掉,然后把这些代码放到**壳代码**的空间中去(而且常伴随着**花指令**)!使原程序的起始代码从壳空间开始执行,然后再JMP回原程序空间。如果我们脱掉壳,这一部分代码就会遗失,也就达到了反脱壳的目的,这就是stolen code(或者stolen bytes)技术,或者更确切的说是stolen OEP code技术。
