...
因此我们知道了上边这个api是GetVersion，入口地址是751A9D30

![image-20251228212811976](../../image-20251228212811976.png)

在ImpREC中打开406ADC这项，把它替换为kernel32.GetVersion。好了，以上是对单个API手动修复的方法，下面介绍自动化方法

### 自动修复：

我们把这个表拖到第=底，选中最后一项按shift然后右键使用tElock0.98插件修复，另外也可以用ImpRec自带的tracer进行修复

### 关键跳法：

#### 异常的IAT项：

仍然是GetVersion这个api，重新加载一下。

当壳代码还没开始执行的时候，406ADC这块内存的值是**3D 83 08 70**，等我们到入口点后，就变成了**F7 06 2F 02**，因此在壳的解密例程中会将这个地址填充到响应内存单元。

为了监控何时会被写入，可以给这块内存设置内存写入断点（因为硬件断点会被检测）

由于不太会用xdbg的内存断点（没断下来），所以此处用Ollydbg演示。

![image-20260104164006092](tElock0.98.assets/image-20260104164006092.png)

经过几次F9之后，到达了这里，可以看到寄存器窗口中正在将ecx中的被重定向过的022306F7正在被写入460ADC、这块内存，所以这是我们需要定位的地方。

按下F8后，地址被保存在460ADC。当我们一步一步F7直到46651A这个地址时，右上角ebx对应的ASCII时GetVersion这个api。

![image-20260104164429761](tElock0.98.assets/image-20260104164429761.png)

这行代码似乎比较关键，我们继续往下看。

![image-20260104164553088](tElock0.98.assets/image-20260104164553088.png)

几步F7后，我们发现了GetProcAddress这个函数，显然是用来获取GetVersion地址的api

![image-20260104164815103](tElock0.98.assets/image-20260104164815103.png)

此时，eax的值正是GetVersion的真实地址，我们接着往下跟。找到了一个jnz跳转，跟进去

![image-20260104165457420](tElock0.98.assets/image-20260104165457420.png)

发现程序并没有把这个地址放到460ADC，而是放到了edi对应的区域022317D4，也就是壳区段。

![image-20260104165740717](tElock0.98.assets/image-20260104165740717.png)

然后是一个非常长的跳转，跳到了最开始的部分，这是一个循环。它通过GetProcAddress获取api地址，并把这个地址存到壳区段，直到所有IAT项都被处理完为止。

#### 正常的IAT项：

依据上面的方法，随便找一个正常的IAT项。然后看看它是怎么被放到IAT中的。

![image-20260104193123471](tElock0.98.assets/image-20260104193123471.png)

依据跟踪异常IAT项的方法跟踪正常的IAT项，发现了一处不同，这里有一个跳转忽略了大部分代码，其地址是4663D2。因此这里决定了IAT是否会被重定向，修改的思路就是改成jmp强制跳过重定向流程。

在最开始，我们就转到4663D2这个地址，

![image-20260104193846924](tElock0.98.assets/image-20260104193846924.png)

发现都是些垃圾数据。那就换一种方法。给IAT表都打上内存写入断点，这样一写入内存，程序就会断下来，那么这个时候这块区域的代码将是正常的。然后按几下F9，再去4663D2看看，发现是正常的。我们直接改成jmp，

![image-20260104194246682](tElock0.98.assets/image-20260104194246682.png)

然后再删除内存写入断点，之后转到OEP处。这下IAT项都正常了

![image-20260104194553658](tElock0.98.assets/image-20260104194553658.png)

他们都是7几开头的。

## 0x03 Dump

用ollydump插件，把程序以不重建输入表的方式dump出来，然后打开ImpRec，填入如下数据，获取IAT

![image-20260104195054851](tElock0.98.assets/image-20260104195054851.png)
...
![image-20260104195309345](tElock0.98.assets/image-20260104195309345.png)
